...
async function addWarning(guildId, userId, reason, moderatorId, severity = 'minor', options = {}) {
    const operationKey = `add:${guildId}:${userId}:${reason}:${severity}:${moderatorId}`;

    // Prevent duplicate operations
    if (!global.warningOperations) {
        global.warningOperations = new Set();
    }

    if (global.warningOperations.has(operationKey)) {
        console.log(`⚠️ Duplicate warning operation detected, skipping: ${operationKey}`);
        return { success: false, message: 'Duplicate operation prevented' };
    }

    global.warningOperations.add(operationKey);

    // Auto-cleanup after 30 seconds
    setTimeout(() => {
        global.warningOperations.delete(operationKey);
    }, 30000);

    try {
        console.log(`📝 Adding warning for ${userId} in guild ${guildId}`);
        ...
        const warning = createWarning(guildId, userId, reason, moderatorId, severity, options.expiresInDays || 0);
        ...
        if (options.client && !warning.logged) {
            try {
                const guild = options.client.guilds.cache.get(guildId);
                if (guild) {
                    const user = await options.client.users.fetch(userId).catch(() => null);
                    const moderatorUser = await options.client.users.fetch(moderatorId).catch(() => null);
                    ...
                    if (user && moderatorUser) {
                        ...
                        try {
                            ...
                            await sendWarningAppealButton(user, guildId, warning.id, reason, severity, options.client);
                        } catch (appealError) {
        ...
        // Ensure the operation key is removed in case of error as well
        if (global.warningOperations.has(operationKey)) {
            global.warningOperations.delete(operationKey);
        }
        throw error;
...
async function processWarningAppeal(guildId, warningId, moderatorReason, userId) {
    try {
        ...
        foundFoundWarning.appealProcessedBy = userId;
...
class WarningSystem {
    ...
    async addWarning(guildId, userId, reason, moderatorId, severity = 'moderate', expiresInDays = 0) {
        return await addWarning(guildId, userId, reason, moderatorId, severity, { expiresInDays, client: this.client });
    }
